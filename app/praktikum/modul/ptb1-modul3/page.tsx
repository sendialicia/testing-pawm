...
export default function PTB1Modul3() {
  ...
  const moduleInfo = {
    title: "PTB1 - Modul 3",
    subtitle: "Peralatan Diagnosa Medis",
    description: "Modul ini membahas tentang peralatan diagnosa medis seperti EKG, USG, dan peralatan radiologi yang digunakan dalam dunia medis.",
    modulId: "ptb1-modul3"
  };

  // Fetch tugas awal status
  const fetchTugasAwalStatus = useCallback(async (userId: string) => {
    try {
      const response = await fetch(`/api/tugas-awal?userId=${userId}&modulId=${moduleInfo.modulId}`);
      const data = await response.json();
      
      if (data.success && data.data && Array.isArray(data.data) && data.data.length > 0) {
        const tugasData = data.data[0];
        // Only set as submitted if linkTugas exists
        if (tugasData.linkTugas && tugasData.linkTugas.trim() !== '') {
          setTugasAwalSubmitted(true);
          setTugasAwalData({
            nama: tugasData.nama || '',
            nim: tugasData.nim || '',
            linkTugas: tugasData.linkTugas || ''
          });
          setTugasAwalNilai(tugasData.nilai || null);
          setTugasAwalNilaiAt(tugasData.nilaiAt || null);
        } else {
          // Data corrupt/incomplete - reset state
          setTugasAwalSubmitted(false);
          setTugasAwalNilai(null);
          setTugasAwalNilaiAt(null);
        }
      }
    } catch (error) {
              ...
              <div className="flex items-start space-x-6 mb-8">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-800 mb-4">
                    Download & Submit Tugas Awal PTB1 - Modul 3
                  </h3>
